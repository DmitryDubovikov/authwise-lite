...
def test_decide(status: str, retry_cycles: int, expected: str) -> None:
    assert decide(_policy(status), retry_cycles=retry_cycles, retry_limit=2) == expected


def test_decide_zero_limit_never_retries() -> None:
    assert decide(_policy("missing_info"), retry_cycles=0, retry_limit=0) == "request-info"
